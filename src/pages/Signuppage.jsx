        ...
        <div className="App">
              <StSignUP>
                ...
                <StForm>
                    <div>
                        <StinputImg/>
                        <StInput style={{width: '200px'}} type="file" placeholder='profile image'/>
                    </div>
                    <div>
                        <StInput onChange = {onChangeUsername} name="username" type="email" placeholder='e-mail'/>
                        <StMsg>{username.length > 0 && <span className={`message ${isUsername ? 'success' : 'error'}`}>
                            {usernameMsg}</span>}</StMsg>

                        <StInput onChange = {onChangeNickname} name="nickname" type="text" placeholder='nickname'/>
                        <StMsg>{nickname.length > 0 && <span className={`message ${isNickname ? 'success' : 'error'}`}>
                            {nicknameMsg}</span>}</StMsg>

                        <StInput onChange = {onChangePassword} name="password" type="password" placeholder='password'/>
                        <StMsg>{password.length > 0 && <span className={`message ${isPassword ? 'success' : 'error'}`}>
                            {passwordMsg}</span>}</StMsg>

                        <StInput onChange = {onChangePasswordConfirm} name="passwordConfirm" type="password" placeholder='password confirm'/>
                        <StMsg>{passwordConfirm.length > 0 && <span className={`message ${isPasswordConfirm ? 'success' : 'error'}`}>
                            {passwordConfirmMsg}</span>}</StMsg>
                    </div>
                </StForm>

                  <div className='signup'>
                    <div style={{padding: '10px'}}></div>
                    <StBtn onClick ={onSubmit} style={{color:'white', backgroundColor: '#49C530', margin:'10px 50px'}}>sign up</StBtn>
                  </div>
              </StSignUP>
        </div>
      );
